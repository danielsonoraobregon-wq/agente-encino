...
Frente al Restaurant El Pariente. 5 min de Pueblo Salvaje, 3 min del Rio Blanquillo, 45 min d
Maps: https://maps.app.goo.gl/y9ske7rVR2nBSS8s9
Caracteristicas: unico proyecto pavimentado en la zona, acceso controlado, electricidad subte
Lotes disponibles:
- Lote 1: 1,648 m2 (38x38m) - $1,600,000 - 24 MSI
- Lote 3: 1,639 m2 (43x38m) - $1,726,200 - 24 MSI
- Lote 3B: 1,700 m2 (43x39m) - $1,785,000 - 24 MSI
- Lote 4 PREMIUM: 1,632 m2 (45x38m) - $1,600,000 - Contado - en colina con la mejor vista del
Financiamiento directo sin banco: Enganche $350,000 mas 24 mensualidades de $75,000 mas pago
Proceso: apartar, contrato notaria, escrituras listas.
Visitas: sabados y domingos.
`;
// ─── SYSTEM PROMPT ───────────────────────────────────────────────
const SYSTEM_PROMPT = `Eres Daniel Soliz, asesor de ventas de Privada Encino en Montemorelos
${INFO_ENCINO}
PERSONALIDAD:
Profesional, directo y cordial. Estilo Monterrey. Mensajes cortos, maximo 3 lineas. Sin lista
REGLA ABSOLUTA - INFORMACION:
SOLO usa la informacion que esta escrita arriba. NUNCA inventes cosas que no esten en el proy
CITAS Y VISITAS - MUY IMPORTANTE:
Si el cliente menciona querer visitar, agendar cita, conocer el terreno, ir a ver, o cualquie
...
- Si pregunta "algo mas barato": "El precio inicial es desde $1,600,000 con financiamiento di
COMO RESPONDER:
1. LEE el historial completo antes de responder. Nunca trates un mensaje como si fuera el pri
2. LEE el primer mensaje con atencion — si dice "precios" responde precios, si dice "ubicacio
3. RESPONDE siempre lo que pregunto el cliente aunque escriba mal. "ubaicon"=ubicacion, "fina
4. NUNCA digas "no entiendo". Siempre responde algo util.
5. UNA sola pregunta por mensaje.
6. Precios: siempre "desde $1,600,000".
7. Cuando piden fotos: "Con gusto, en un momento le comparto algunas fotos del proyecto." Sin
REGLA DE ORO - NUNCA DES TODO JUNTO:
Si el cliente pide precios, ubicacion, medidas y financiamiento en el mismo mensaje, responde
MENSAJES EN 2 PARTES:
Cuando necesites dar informacion Y hacer una pregunta, separa con ---
Ejemplo:
Estamos en Montemorelos, 45 min de Monterrey. Aqui el mapa: https://maps.app.goo.gl/y9ske7rVR
---
...
HORARIO: L-V 9am-9pm, S-D tambien. Fuera de horario: "Gracias por escribir, con gusto le atie
...
if (!seg.alertaEnviada && ahora - seg.timestamp > 172800000) {
        await mandarAlerta("Lead 48hrs: " + seg.subscriberId); seg.alertaEnviada = true;
...
return res.json({ respuesta1: null, respuesta2: null, alerta: "congelado", foto: fals
}
// Fuera de horario
if (!dentroDeHorario()) {
procesando.delete(clave);
...
conversacion.push({
role: "user",
content: "[El cliente llego por un anuncio y su primer mensaje fue: " + primer_mens
});
} else {
...
procesando.delete(clave);
return res.json({ respuesta1: "Un momento, dejame verificarlo.", respuesta2: null, al
}
...
if (nombre === "calificado") await mandarEventoMeta("CompleteRegistration", telefon
}
...
await mandarAlerta("VISITA PENDIENTE\nCliente: " + (telefono || subscriber_id) await guardarVisita(clave, detalle);
...
await mandarAlerta("AUDIO recibido\nCliente: " + (telefono || subscriber_id) + await setBotCongelado(clave, true);
"\nRes
...
res.json({ status: "Historial limpiado", conversaciones_borradas: claves.length });
} catch (e) {
res.status(500).json({ error: e.message });
}
});
