...
PRECIOS:
Cuando mencionas precios SIEMPRE menciona el precio original tachado y el precio actual. Ejemplo: "El Lote 1 tiene un precio de lanzamiento de $1,700,000, precio original $2,000,000."
...
Si el cliente pide precios, ubicacion, medidas y financiamiento en el mismo mensaje, responde SOLO la ubicacion con el link del mapa en el primer mensaje, y en el segundo pregunta que le interesa mas.

MENSAJES EN 2 O 3 PARTES:
Cuando necesites dar informacion Y hacer una pregunta, separa con ---
Cuando necesites dar informacion en 3 bloques separados (por ejemplo: saludo, detalle, pregunta), usa --- dos veces para separar los 3 bloques.
...
PDF_ENCINO: SOLO cuando el cliente califica (tiene intencion clara Y plazo definido) o pide info para compartir con familiar
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    try {
      ...
      procesando.delete(clave);
      res.json({ respuesta1, respuesta2, respuesta3, alerta: alerta || null, foto });
...
app.get("/limpiar", async (req, res) => {
  try {
    ...
    const visitas = await redis.keys("visita:*");
    for (const c of visitas) await redis.del(c);
    res.json({ status: "Todo limpiado", conversaciones_borradas: claves.length, leads_borrados: leads.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/", (req, res) => {
  res.json({ status: "Agente Daniel - Privada Encino v3.1 funcionando" });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, function () {
  console.log("Servidor Daniel v3.1 corriendo en puerto " + PORT);
});
