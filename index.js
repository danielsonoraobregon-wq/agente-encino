...
async function setConversacion(clave, mensajes) {
  try {
    ...
    const valor = JSON.stringify(mensajes);
    await redis.set(key, valor, { ex: 86400 });
    
    // Verificar que se guardo correctamente
    const check = await redis.get(key);
    if (check) {
      const checkParsed = typeof check === "string" ? JSON.parse(check) : check;
      const len = Array.isArray(checkParsed) ? checkParsed.length : 0;
      console.log("REDIS SAVE OK: " + key + " → " + len + " mensajes guardados y verificados");
    } else {
      console.error("REDIS SAVE FALLÓ: " + key + " → se guardó pero verificación devolvió null");
    }
  } catch (e) {
