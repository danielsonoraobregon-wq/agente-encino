...
async function setProcesando(clave) {
  const result = await redis.set("proc:" + clave, "1", { nx: true, ex: 30 });
...
- Lote 1 y 3B (18 MSI): Enganche $400,000 + 18 mensualidades de $50,000 + pago final $400,000 al liquidar
- Lote 4 PREMIUM (12 MSI): Enganche $400,000 + 12 mensualidades de $83,000 aprox
...
Si el cliente pide precios, ubicacion, medidas y financiamiento en el mismo mensaje:
- Mensaje 1: ubicacion con link del mapa
- Mensaje 2: precios de los 3 lotes con tachado
- Mensaje 3: pregunta que le interesa mas o siguiente paso
Usa ||| para separar cada bloque.
...
VIDEO_COLINA: cuando el cliente pregunta especificamente por el Lote 4 PREMIUM, la colina, o quiere ver la vista
...
async function setConversacion(clave, mensajes) {
  try {
    ...
    if (mensajes.length > 20) {
      ...
      const cola   = mensajes.slice(-18);    // últimos 18
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    try {
      ...
      if (congelado) {
        // FIX #12 — cliente escribe mientras congelado, no silencio total
        await delProcesando(clave);
        return res.json({
          respuesta1: "Su mensaje fue recibido, en breve le atendemos personalmente.",
          respuesta2: null, respuesta3: null, alerta: "congelado"
        });
      }
      ...
      if (esNuevo) {
        ...
        // Videos primero — delay 2s para que lleguen ANTES que el texto
        if (subscriber_id) {
