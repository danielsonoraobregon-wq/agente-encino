...
FLUJO:
- Si el historial esta vacio y el cliente solo dice "hola" o saludo sin pedir nada: presentate como Daniel Soliz y pregunta que busca.
...
PDF_ENCINO: SOLO cuando el cliente EXPLICITAMENTE pida un PDF, folleto, informacion para compartir con alguien, o diga "pasame algo para ensenarle a mi esposa/familia". NUNCA mandes el PDF solo porque el cliente tiene intencion de compra. El PDF es para compartir, no para informar.
    ...
    try {
      ...
      if (esNuevo) {
        ...
        if (subscriber_id) {
          ...
          console.log("VIDEOS enviados para:", clave);
        } else {
      ...
      if (respuesta.includes("ALERTA_VISITA_PENDIENTE")) {
        ...
        if (subscriber_id) await ponerEtiqueta(subscriber_id, "seguimiento-pendiente");
      }
      ...
      if (etiquetasMatch && etiquetasMatch.includes("ETIQUETA:calificado")) {
        console.log("LEAD CALIFICADO:", telefono || subscriber_id);
      }
...
app.get("/", (req, res) => {
  res.json({ status: "Agente Daniel - Privada Encino v3.5 funcionando" });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, function () {
  console.log("Servidor Daniel v3.5 corriendo en puerto " + PORT);
});
